...
export default function UserDashboard() {
  ...
  const handleRenameFolder = async (folderId: string, newName: string) => {
    ...
    try {
      const res = await fetch(`/api/user/folders/rename`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteFolder = async (folderId: string) => {
    ...
    try {
      const res = await fetch(`/api/user/folders/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  const handleArchiveFolder = async (folderId: string) => {
    try {
      const res = await fetch(`/api/user/folders/archive`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ folderId }),
      });
  ...
  const handleUnarchiveFolder = async (folderId: string) => {
    try {
      const res = await fetch(`/api/user/folders/unarchive`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ folderId }),
      });
  ...
  const handlePasteItems = async (targetFolderId: string | null) => {
    ...
    try {
      ...
      if (clipboardOperation === "cut") {
        // Move items
        const response = await fetch("/api/user/files/bulk-move", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            items: itemsToMove,
        ...
        // Copy items
        const response = await fetch("/api/user/files/bulk-copy", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            items: itemsToMove,
